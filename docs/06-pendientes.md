...
| P-09 | **P1** | **Modo 2.5D**: terminar la fase A (tareas 8–12 del plan) y seguir con B–E. Rama `modo-25d-fase-a` (no está en `origin`). Todo en [08](08-traspaso-opencode.md) | Fase A: troceo en módulos (`art/water/fx/vision/chars/camera/input/terrain/world`), `engine.js` desaparece, docs 01–07 al día, desplegado |
| P-10 | P2 | Menores diferidos de la fase A: `.tag` con `var(--amber-ink)`; `img/packs25.png` relativo a la página; test de iniciativa con tiempos ajustados (66/67 una vez) | Revisión final de la rama |
