...
4. **«Terreno a plena luz con entorno Interior» no es un bug del port**: el diorama original hace lo mismo en vista Director (comprobado ejecutándolo en el mismo Edge headless: sólo cambian fondo, sol y sombras). La oscuridad real aparece en vistas Grupo/personaje, que llegan en la fase C. Si en C con fichas reales no oscurece, revisar `vision.js` (`uFogOn`, `uFloor`).
