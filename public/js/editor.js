...
function endPointer(e){
  ...
  switch(A.kind){
    ...
    case 'move':
      ...
      break;
    case 'lightRadius':case 'wallEnd':case 'resize':case 'zoneResize':case 'zoneVertex':case 'rotate':if(A.moved){pushUndo(A.snap);changed(true)}break;
  }
  requestRender();
}
...
stage.addEventListener('dragover',e=>{const t=e.dataTransfer.types;if(isGM()&&(t.includes('text/x-light')||t.includes('text/x-image')||t.includes('Files'))){e.preventDefault();e.dataTransfer.dropEffect='copy'}});
...
function openEditor(o,sp){
  ...
  const lightFields=(L,isToken)=>{
    ...
    if(!gm&&isToken){check('Encendida',L.on,v=>L.on=v);return}
...
$('#ambient').onchange=()=>{pushUndo(ambSnap||undefined);ambSnap=null;changed()};
