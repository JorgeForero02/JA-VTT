...
const Net=(()=>{
  ...
  function handle(d){
    switch(d.t){
      ...
      case 'terrain':if(!UI.scene||d.scene!==UI.scene.id||!window.D3)break;if(d.full)window.D3.loadTerrain(d.full);else if(d.op&&!window.D3.applyRemoteOp(d.op,d.version))Net.sendRaw({t:'terrain',scene:UI.scene.id,want:'full'});if(d.fix)toast(d.error||'No puedes editar el terreno de esta escena');if(typeof render25Panel==='function')render25Panel();if((d.full||(d.op&&d.op.type==='art'))&&typeof renderArte25==='function')renderArte25();break;
