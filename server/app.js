...
const IMAGE_CATEGORIES = ['board', 'prop', 'pc', 'npc', 'arte25'];
...
async function handleFogOf(b, c, d) {
  ...
  const pre = b.settings.mode === '2.5d' ? FOG_RAW : FOG_PNG;   // en 2.5D la niebla es un byte por casilla, no un PNG
  const fog = (await q.fogFor(c.sceneId, d.user)).map((r) => ({ cx: r.cx, cy: r.cy, data: pre + r.data.toString('base64') }));
